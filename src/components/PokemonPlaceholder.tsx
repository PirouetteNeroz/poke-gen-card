...
import { Download, Loader2, Search, Sparkles, FileText } from "lucide-react";
...
const PokemonPlaceholder = () => {
  ...
  const searchByPokemon = useCallback(async () => {
    if (!pokemonName.trim()) {
    ...
    try {
      ...
      const cardsResponse = await fetch(
        `https://api.tcgdex.net/v2/en/cards?name=${encodeURIComponent(pokemonName)}`
      );
      ...
      // Sort by release order then card number
      filtered.sort((a, b) => {
      ...
      if (filtered.length === 0) {
        toast.info("Aucune carte trouvée pour ce Pokémon");
      } else {
        toast.success(`${filtered.length} cartes trouvées pour ${pokemonName}`);
      }
    } catch (error) {
      console.error("Error searching by pokemon:", error);
  ...
  const generatePDF = useCallback(async () => {
    ...
    try {
      ...
      doc.text(pokemonName, pageWidth / 2, pageHeight / 2 - 15, { align: "center" });
      doc.setFontSize(16);
      ...
      // Draw cut lines (skip title page)
      ...
      doc.save(`placeholder_${pokemonName.replace(/\s+/g, "_")}.pdf`);
      setProgress(100);
        ...
        <CardContent className="space-y-6">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <Input
                placeholder="Nom du Pokémon (ex: Charizard, Pikachu)"
                value={pokemonName}
                onChange={(e) => setPokemonName(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && searchByPokemon()}
              />
            </div>
            <Button
              onClick={searchByPokemon}
              disabled={isLoading}
              variant="outline"
            >
              {isLoading ? (
          ...
          {/* Pokémon populaires */}
          <div className="space-y-2">
            <p className="text-sm font-medium text-muted-foreground">Pokémon populaires :</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="text-center shadow-card hover:shadow-pokemon transition-all duration-300">
          <CardContent className="pt-6">
            ...
            <h3 className="font-semibold text-lg mb-2">Par Pokémon</h3>
            <p className="text-muted-foreground text-sm">
              Recherchez toutes les cartes d'un Pokémon spécifique
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="text-center shadow-card hover:shadow-pokemon transition-all duration-300">
          <CardContent className="pt-6">
            ...
            <p className="text-muted-foreground text-sm">
              Accédez rapidement aux Pokémon populaires
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="text-center shadow-card hover:shadow-pokemon transition-all duration-300">
          <CardContent className="pt-6">
            ...
            <p className="text-muted-foreground text-sm">
              Cartes triées par date de sortie du set
            </p>
          </CardContent>
        </Card>
      </div>
